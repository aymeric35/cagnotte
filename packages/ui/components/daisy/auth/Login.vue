...
<template>
  <DaisyAuthWrapper>
    ...
    <template v-if="hasProvider">
      <DaisyAuthGoogleButton :loading="withGoogleLoading" :disabled="loginForm.loading || withGoogleLoading"
    ...
    <template v-if="forgotPasswordIsActive">
      <DaisyForm v-model="forgotPasswordForm.valid" :disabled="forgotPasswordForm.loading || form.disabled"
        ...
        <DaisyAuthEmailInput v-model="forgotPasswordForm.data.email" />
      ...
      <label class="label text-xs justify-start space-x-1">
        <DaisyLink :disabled="loginForm.loading || form.disabled" @click="forgotPasswordIsActive = false">
    ...
    <template v-else>
      <DaisyForm v-model="loginForm.valid" :disabled="loginForm.loading || form.disabled" @submit="submitLoginForm">
        <DaisyAuthEmailInput v-model="loginForm.data.email" />
        <DaisyAuthPasswordInput v-model="loginForm.data.password" />

        <label class="label">
          <DaisyLink :disabled="loginForm.loading || form.disabled" @click="forgotPasswordIsActive = true">Mot de
            passe oublié ?
          </DaisyLink>
        </label>
