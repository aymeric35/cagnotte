...
type lazyRule = boolean | 'ondemand';
interface UseValidateProps {
    ...
    errorMessage?: String,
    noErrorIcon?: Boolean,
    rules?: Array<Promise<boolean> | string | Function>,
...
export default function () {
    ...
    if (!currentInstance) throw new Error("getCurrentInstance() return null")
    const { proxy } = currentInstance;
    const props: UseValidateProps = currentInstance.props;

    const innerError = ref<boolean>(false)
    const innerErrorMessage = ref<string | null>(null)
    ...
    let validateIndex = 0;
    let unwatchRules: ReturnType<typeof watch> | undefined = undefined;
    ...
    function validate(val: any = props.modelValue): boolean | Promise<boolean> {
        ...
        const update = (err: boolean, msg?: string) => {
            err === true && setDirty()
